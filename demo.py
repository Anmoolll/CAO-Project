...
while running:
       ...
       # Ball collision with walls
        ...
        if ball.left <= 0:
           ...
           if right_score >= 3:
               ...
               BALL_SPEED_X += 5
               BALL_SPEED_Y += 5
               # Check if ball speed is between 60 and 80
               BALL_SPEED_X = max(min(BALL_SPEED_X, 80), 60)
               BALL_SPEED_Y = max(min(BALL_SPEED_Y, 80), 60)
               ball = pygame.Rect(WIDTH // 2 - BALL_SIZE // 2, HEIGHT // 2 - BALL_SIZE // 2, BALL_SIZE, BALL_SIZE)
               ball_speed = [random.choice((BALL_SPEED_X, -BALL_SPEED_X)), random.choice((BALL_SPEED_Y, -BALL_SPEED_Y))]
    ...
    if game_paused and winner:
        ...
        # Restart the game when 't' is pressed
        if keys[pygame.K_t]:
            left_score = 0
            right_score = 0
            game_paused = False
